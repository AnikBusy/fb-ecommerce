            ...
            <div className="relative">
                <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 md:gap-10">
                    {categories.slice(0, 6).map((cat) => (
                        ...
                        >
                            <div className="relative w-full aspect-square rounded-full overflow-hidden border border-border shadow-md p-1 bg-card group-hover:border-mongodb-green group-hover:shadow-[0_20px_40px_rgba(0,237,100,0.2)] transition-all duration-500 flex items-center justify-center">
                                {cat.image ? (
                                    <img
                                        src={cat.image}
                                        alt={cat.name}
                                ...
                                {/* Inner glow on hover */}
                                <div className="absolute inset-0 rounded-full bg-mongodb-green/0 group-hover:bg-mongodb-green/5 transition-colors duration-500"></div>
                            </div>
                            <div className="flex flex-col items-center gap-3">
                                <span className="text-[11px] font-black uppercase tracking-[0.3em] text-foreground/80 group-hover:text-mongodb-green transition-colors duration-300">
                                    {cat.name}
                                </span>
                                <div className="h-[3px] w-0 bg-mongodb-green group-hover:w-8 transition-all duration-500 rounded-full shadow-[0_0_10px_rgba(0,237,100,0.3)]"></div>
                            </div>
                        </Link>
                    ))}
                </div>
