...
export async function sendPurchaseEvent(order) {
    ...
    const isPixelActive = settings.isPixelActive;

    if (!isPixelActive || !pixelId || !accessToken) {
        if (isPixelActive) console.warn('Facebook CAPI: Missing credentials in Settings');
        return;
    }
